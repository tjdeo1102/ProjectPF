...
public class KSH_NetworkVoiceChat2 : MonoBehaviour
{
    // 보이스 챗 마이크 활성화 여부
    ...
    private void Awake()
    {
        ...
        // 상위 객체에서 PhotonVoiceView를 가져옴
        this.photonVoiceView = this.GetComponentInParent<PhotonVoiceView>();
    }

    void Start()
    {
        StartCoroutine(InitializationOperation());
    }
    ...
    public void VoiceChatOnOff()
    {
        VoiceSwitchOnClick();
    }

    private IEnumerator InitializationOperation()
    {
